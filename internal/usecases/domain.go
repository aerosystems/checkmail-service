...
func (du *DomainUsecase) CreateDomain(domainName, domainType, domainCoverage string) (*models.Domain, error) {
	...
	domain := &models.Domain{
		...
		Match: models.DomainCoverageFromString(domainCoverage),
	}
...
func (du *DomainUsecase) UpdateDomain(domainName string, domainType, domainCoverage string) (*models.Domain, error) {
	domain, err := du.domainRepo.FindByName(domainName)
	if err != nil {
		return nil, err
	}
	domain.Type = models.DomainTypeFromString(domainType)
	domain.Match = models.DomainCoverageFromString(domainCoverage)
	if err := du.domainRepo.Update(domain); err != nil {
		return nil, err
	}
	return domain, nil
}
...
func (du *DomainUsecase) CountDomains() (map[models.Type]int, error) {
	// TODO: add cache
